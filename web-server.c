...
void cerror(FILE *stream, char *cause, char *errno, 
  ...
  fprintf(stream, "<hr><em>The Http Web server</em>\n");
}
...
int main(int argc, char **argv) {
  ...
  if (argc != 1) {
    fprintf(stderr, "usage: ./web-server\n", argv[0]);
    exit(1);
  }
  /*portno = atoi(argv[1]); */
  ...
  while (1) {
    ...
    /* server only supports the GET method */
	...
	*p = '\0';
	...
	strcpy(filetype, "text/plain");
      ...
      fprintf(stream, "Server: Http Web Server\n");
	...
	continue;
      ...
      sprintf(buf, "Server: Http Web Server\n");
      write(childfd, buf, strlen(buf));
